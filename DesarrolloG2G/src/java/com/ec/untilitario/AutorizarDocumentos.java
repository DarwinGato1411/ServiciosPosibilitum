...
public class AutorizarDocumentos {
    ...
    public AutorizarDocumentos(Tipoambiente tipoambiente) {
 ...
 /*  62 */ if (clave.toString().length() != 49) {
    ...
    public RespuestaSolicitud validar(byte[] datos) {
        try {
            ...
            URL url = new URL("https://" + tipoambiente.getAmUrlsri() + "/comprobantes-electronicos-ws/RecepcionComprobantesOffline?wsdl");
...
//            build.append(linea);
            ...
            linea = ("    <infoAdicional>\n"
                       ...
                       + (amb.getAmRipme()? "<campoAdicional nombre=\"CONTRIBUYENTE REGIMEN RIMPE\">CONTRIBUYENTE REGIMEN RIMPE</campoAdicional>\n" : "<campoAdicional nombre=\"CONTRIBUYENTE REGIMEN GENERAL\">CONTRIBUYENTE REGIMEN GENERAL</campoAdicional>\n")
                        + "   </infoAdicional>\n"
                        + "</factura>\n");
            build.append(linea);
    ...
    public String generaXMLNotaCreditoDebito(NotaCreditoDebito valor, Tipoambiente amb, String folderDestino, String nombreArchivoXML, String NCoND) {
        ...
        try {
            ...
            String linea = ("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n"
                        ...
                        + "              </totalImpuesto>\n"
                        + "        </totalConImpuestos>\n"
                        + "        <motivo>" + removeCaracteres(valor.getFacDescripcion().trim()) + "</motivo>\n" //Motivo
                        + "    </infoNota" + tipoDocumento + ">\n");
            build.append(linea);
...
//                List<Detalledocumento> det = detalledocumentoFacade.getDetalleDcto(cabdoc.getIdcabeceradocumentos());
                ...
                for (DetalleNotaDebitoCredito item : listaDetalle) {
                    ...
                    linea = ("        <detalle>\n"
                                + "            <codigoInterno>" + removeCaracteres(item.getCodigoProducto()) + "</codigoInterno>\n"
                                + "            <descripcion>" + removeCaracteres(item.getDetDescripcion()) + "</descripcion>\n"
                                + "            <cantidad>" + item.getDetCantidad().setScale(2, RoundingMode.FLOOR) + "</cantidad>\n"
                                ...
                                + "                <impuesto>\n"
                                + "                    <codigo>" + valor.getFacCodIva() + "</codigo>\n"
                                + "                    <codigoPorcentaje>" + (item.getProdGrabaIva() ? "2" : "0") + "</codigoPorcentaje>\n"
                                + "                    <tarifa>" + (item.getProdGrabaIva() ? "12" : "0") + "</tarifa>\n"
...
//                    : generaClaveContingencia(retencion.getFechaemision(), retencion.getCodigodcto(), tipoemision);
    ...
    //<editor-fold defaultstate="collapsed" desc=" ARMAR LIQUIDACION DE COMPRA"> 
    public String generaXMLLiquidacion(Factura valor, Tipoambiente amb, String folderDestino, String nombreArchivoXML, Boolean autorizada, Date fechaAutorizacion) {
        try {
            FileOutputStream out = null;
            SimpleDateFormat formato = new SimpleDateFormat("dd/MM/yyyy");
            ...
            linea = ("<infoTributaria>\n"
                        ...
                        + (amb.getContriEsp() ? "        <contribuyenteEspecial>" + (amb.getContriEsp() ? amb.getAmNroContribuyente() : "") + "</contribuyenteEspecial>\n" : "")
                        + "        <obligadoContabilidad>" + amb.getLlevarContabilidad() + "</obligadoContabilidad>\n"
                        + "        <tipoIdentificacionProveedor>" + valor.getIdCliente().getIdTipoIdentificacion().getTidCodigo() + "</tipoIdentificacionProveedor>\n"
                        + "        <razonSocialProveedor>" + removeCaracteres(valor.getIdCliente().getCliNombre()) + "</razonSocialProveedor>\n"
                        + "        <identificacionProveedor>" + valor.getIdCliente().getCliCedula() + "</identificacionProveedor>\n"
                        + "        <totalSinImpuestos>" + ArchivoUtils.redondearDecimales(valor.getFacSubtotal(), 2) + "</totalSinImpuestos>\n"
                        + "        <totalDescuento>" + valor.getFacDescuento().setScale(2, RoundingMode.FLOOR) + "</totalDescuento>\n"
                        + "        <codDocReembolso>41</codDocReembolso>\n"
                        + "        <totalComprobantesReembolso>41</totalComprobantesReembolso>\n"
                        + "        <totalBaseImponibleReembolso>41</totalBaseImponibleReembolso>\n"
                        + "        <totalImpuestoReembolso>41</totalImpuestoReembolso>\n"
                        + "        <totalConImpuestos>\n"
                        + "            <totalImpuesto>\n"
                        + "                <codigo>" + valor.getFacCodIva() + "</codigo>\n"
                        + "                <codigoPorcentaje>0</codigoPorcentaje>\n"
